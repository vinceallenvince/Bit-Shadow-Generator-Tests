(function() {
 ...
 'use strict';
  ...
  function init(gen) {
    generator = gen;
    generator.addMenuItem('TestText', 'TestText', true, false);
  ...
  function menuClicked(e) {
    if (e.generatorMenuChanged.name === 'TestText') {
  ...
  var createDoc = function() {
    ...
    for (var i = 0, max = items.length; i < max; i++) {
      ...
      layerRef.textItem.contents = "Hi!";
      var fontColor = new SolidColor();
      fontColor.rgb.red = 255;
      fontColor.rgb.green = 255;
      fontColor.rgb.blue = 255;
      layerRef.textItem.color = fontColor;
      layerRef.textItem.font = myFont;
      layerRef.textItem.size = 100;
      layerRef.textItem.position = Array(x, y);

      app.activeDocument.activeLayer.opacity = constrain(item.opacity * 100, 0, 100);
      if (blurItems) {
        var blurAngle = radiansToDegrees(Math.atan2(item.velocity.y, item.velocity.x));
        // PS wants values bw -90 and 90
        blurAngle *= -1; // flip the coordinate system
        if (blurAngle >= 90 && blurAngle <= 180) {
          blurAngle -= 180;
        } else if (blurAngle <= -90 && blurAngle >= -180) {
          blurAngle += 180;
        }
